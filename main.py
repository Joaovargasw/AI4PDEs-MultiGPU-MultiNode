    ...
    with torch.no_grad():
        for itime in range(1, ntime + 1):
            ...
            if save_fig(itime, n_out):
                ...
                end_save = time.time()
                save_time_accumulator += end_save - start_save
    end = time.time()
    if DEBUG_PRINTS:
        print_gpu_stats(rank)
...
def calculate_max_nlevel(nx_global, ny_global, nz_global, world_size):
    ...
    local_nx = nx_global // 2
    local_ny = ny_global // 2
...
def print_gpu_stats(rank):
    device_id = torch.cuda.current_device()

    # Propriedades físicas da GPU
    prop = torch.cuda.get_device_properties(device_id)
    total_memory = prop.total_memory / 1024**3  # Convertendo para GB

    # Memória sendo usada agora (reservada pelo cache do PyTorch)
    reserved = torch.cuda.memory_reserved(device_id) / 1024**3

    # O máximo que o código já usou desde o início
    max_mem = torch.cuda.max_memory_allocated(device_id) / 1024**3

    print(f"[Rank {rank}] {prop.name} | Total: {total_memory:.2f}GB | "
          f"Pico Usado: {max_mem:.2f}GB | Disponível Real: {(total_memory - max_mem):.2f}GB")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save', type=int, default=0, choices=[0, 1], help='Salvar resultados (1) ou não (0)')
    parser.add_argument('--debug', type=int, default=0, choices=[0, 1],help='Debug prints (1) ou não (0)')
    # torch.distributed.run pode adicionar argumentos como --local_rank
    # Vamos ignorar argumentos desconhecidos
    ...
    if rank == 0 and DEBUG_PRINTS:
        ...
        print(
            f"Global z domain: {nz}, World size: {world_size}, Local Z length: {nz // world_size}"
        )

        if nz % world_size != 0:
            print(
                f"Aviso Crítico: nz ({nz}) não é perfeitamente divisível por world_size ({world_size})."
            )
            print(
                "Isso pode causar erros na troca de halos (halo_exchange). Ajuste nz ou o número de GPUs."
            )
