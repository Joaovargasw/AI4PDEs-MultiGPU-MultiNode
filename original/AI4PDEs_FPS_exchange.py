...
###############################################################
    ...
    def PG_vector(self, rank, values_uu, values_vv, values_ww, values_u, values_v, values_w, k1, k_uu, k_vv, k_ww, sigma):#, ADx_u, ADy_u, ADz_u, ADx_v, ADy_v, ADz_v, ADx_w, ADy_w, ADz_w):
        ...
        #k_v = torch.tensor(torch.ones_like(values_v), device=f"cuda:{rank}")
        #k_w = torch.tensor(torch.ones_like(values_w), device=f"cuda:{rank}")
        #k_u = torch.ones_like(values_u, device=f"cuda:{rank}")
        #k_v = torch.ones_like(values_v, device=f"cuda:{rank}")
        #k_w = torch.ones_like(values_w, device=f"cuda:{rank}")
...
def train(rank,world_size,values_w1,values_u1,values_v1,values_p1):
    ...
    if LIBM == True:
        ...
        for i in range(nx):
            for j in range(ny):
                for k in range(nz):
                    dist = ((i * dx - 2)**2 + (j * dy - 2)**2 + (k * dz - 2)**2)**0.5
                    if dist <= 0.5:
                        sigma[0,0,k,j,i] = 1e08

    plt.imshow(sigma.cpu()[0,0,160,:,:])
    plt.colorbar()
